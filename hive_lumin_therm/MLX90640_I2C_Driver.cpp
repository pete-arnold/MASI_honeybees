...
int MLX90640_I2CRead(uint8_t _deviceAddress, unsigned int startAddress, unsigned int nWordsRead, uint16_t *data)
{
  ...
  while (bytesRemaining > 0)
  {
    ...
    Wire.write((unsigned byte)(startAddress >> 8)); //MSB
    Wire.write((unsigned byte)(startAddress & 0xFF)); //LSB
...
int MLX90640_I2CWrite(uint8_t _deviceAddress, unsigned int writeAddress, uint16_t data)
{
  ...
  Wire.write((unsigned byte)(writeAddress >> 8)); //MSB
  Wire.write((unsigned byte)(writeAddress & 0xFF)); //LSB
  Wire.write((unsigned byte)(data >> 8)); //MSB
  Wire.write((unsigned byte)(data & 0xFF)); //LSB
